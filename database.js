    ...
    return {
      get(...params) {
        try {
          ...
          stmt.free();
        } catch (e) { console.error('get error:', sql, e.message); }
        return undefined;
      },
      all(...params) {
        try {
          ...
          stmt.free();
          return rows;
        } catch (e) { console.error('all error:', sql, e.message); }
        return [];
      },
      run(...params) {
        try {
          db.run(sql, params);
          saveDB();
        } catch (e) { console.error('run error:', sql, e.message); }
      }
    };
  },
  exec(sql) { return db ? db.exec(sql) : []; }
};

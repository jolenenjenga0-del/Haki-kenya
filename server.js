...
app.get('/', (req, res) => {
  ...
  res.render('index', { user, stats: { jobs: jobCount.c, applicants: applicantCount.c, placements: placementCount.c } });
});
...
app.get('/dashboard', requireAuth, (req, res) => {
  ...
  res.render('dashboard', { user, applicant, applications, openJobs: openJobs.c, applied: req.query.applied });
});
...
app.post('/jobs/:id/apply', requireAuth, async (req, res) => {
  ...
  const hash = generateBlockchainHash({
    job: job.id, applicant: applicant.id, score: result.score, timestamp: Date.now()
  });
...
app.get('/recruiter/dashboard', requireRole('recruiter'), (req, res) => {
  ...
  const stats = {
    ...
    totalApps: db.prepare(`SELECT COUNT(*) as c FROM applications a JOIN jobs j ON a.job_id = j.id WHERE j.recruiter_id = ?`).get(user.id).c,
    shortlisted: db.prepare(`SELECT COUNT(*) as c FROM applications a JOIN jobs j ON a.job_id = j.id WHERE j.recruiter_id = ? AND a.shortlisted = 1`).get(user.id).c
  };
...
app.post('/recruiter/applications/:id/shortlist', requireRole('recruiter'), (req, res) => {
  ...
  const app = db.prepare(`
    SELECT a.*, j.recruiter_id, j.id as job_id
    FROM applications a JOIN jobs j ON a.job_id = j.id WHERE a.id = ?`).get(req.params.id);
  ...
  const applicantCode = db.prepare(`
    SELECT u.unique_code FROM applications a
    JOIN applicants ap ON a.applicant_id = ap.id
    JOIN users u ON ap.user_id = u.id WHERE a.id = ?`).get(app.id);

  const criteria = req.body.criteria || 'AI Screening Score';

  const hash = generateBlockchainHash({
    job: app.job_id, applicant: applicantCode.unique_code, action: 'shortlisted', timestamp: Date.now()
  });
...
app.get('/public/shortlist', (req, res) => {
  const jobs = db.prepare(`SELECT DISTINCT j.id, j.title, j.department FROM jobs j
    INNER JOIN shortlist_public s ON j.id = s.job_id ORDER BY j.title`).all();
  ...
  if (selectedJobId) {
    entries = db.prepare(`
      SELECT s.*, j.title as job_title, j.department FROM shortlist_public s
      JOIN jobs j ON s.job_id = j.id WHERE s.job_id = ? ORDER BY s.score DESC`).all(selectedJobId);
...
app.get('/admin', requireRole('admin'), (req, res) => {
  ...
  const stats = {
    ...
    applicants: db.prepare('SELECT COUNT(*) as c FROM users WHERE role = ?').get('applicant').c,
    recruiters: db.prepare('SELECT COUNT(*) as c FROM users WHERE role = ?').get('recruiter').c,
    jobs: db.prepare('SELECT COUNT(*) as c FROM jobs').get().c,
    applications: db.prepare('SELECT COUNT(*) as c FROM applications').get().c,
    shortlisted: db.prepare('SELECT COUNT(*) as c FROM applications WHERE shortlisted = 1').get().c
  };
  const auditLog = db.prepare(`
    SELECT s.*, j.title FROM shortlist_public s JOIN jobs j ON s.job_id = j.id ORDER BY s.created_at DESC LIMIT 50`).all();
  res.render('admin', { user, stats, auditLog });
});

// === API: AI analysis for recruiter review ===
app.get('/api/applications/:id/analysis', requireRole('recruiter'), (req, res) => {
  const app = db.prepare(`
    SELECT a.*, u.unique_code, ap.skills, ap.education, ap.experience, j.title, j.requirements, j.description
    FROM applications a
    JOIN applicants ap ON a.applicant_id = ap.id
    JOIN users u ON ap.user_id = u.id
  ...
  res.json({
    applicantCode: app.unique_code,
    score: app.ai_score,
    summary: app.ai_summary,
    criteria: JSON.parse(app.criteria_matched || '[]'),
    skills: app.skills,
    education: app.education,
    experience: app.experience,
    cvText: app.cv_text?.substring(0, 2000),
    jobTitle: app.title
  });
});
